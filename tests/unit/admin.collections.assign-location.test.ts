...
function createJsonRequest(
  body: unknown = {},
  url = 'http://localhost/api/admin/collections/col-1/location'
) {
  return {
    url,
    headers: {
...
describe('POST /api/admin/collections/[collectionId]/location', () => {
  ...
  beforeEach(() => {
    ...
    prismaAssignMock.collection.update.mockResolvedValue({
      ...
      order_index: '1.0',
      captured_at: new Date('2024-01-01T00:00:00Z'),
  ...
  test('returns 400 when collectionId is invalid', async () => {
    const res: typeof NextResponseMock = await route.POST(createJsonRequest(), {
      params: Promise.resolve({ collectionId: 'invalid' }),
    });
  ...
  test('returns 400 when locationId format invalid', async () => {
    const res: typeof NextResponseMock = await route.POST(
      createJsonRequest({ locationId: 'not-uuid' }),
      { params: Promise.resolve({ collectionId }) }
    );
    expect(res.status).toBe(400);
    expect(res.body).toEqual(expect.objectContaining({ field: 'locationId' }));
  ...
  test('returns 404 when collection not found', async () => {
    ...
    const res: typeof NextResponseMock = await route.POST(createJsonRequest({ locationId }), {
      params: Promise.resolve({ collectionId }),
    });
  ...
  test('returns 400 when location belongs to different year', async () => {
    prismaAssignMock.location.findUnique.mockResolvedValueOnce({
      id: locationId,
      year_id: 'year-2',
    });
    const res: typeof NextResponseMock = await route.POST(createJsonRequest({ locationId }), {
  ...
  test('assigns location successfully', async () => {
    ...
    expect(logAudit).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'link', entity: `collection/${collectionId}` })
    );
  ...
  test('allows unassigning location', async () => {
    prismaAssignMock.collection.findUnique.mockResolvedValueOnce({
      id: collectionId,
      year_id: 'year-1',
      location_id: locationId,
    });
    ...
    const res: typeof NextResponseMock = await route.POST(createJsonRequest({ locationId: null }), {
      params: Promise.resolve({ collectionId }),
    });
    expect(res.status).toBe(200);
    expect(prismaAssignMock.collection.update).toHaveBeenCalledWith({
