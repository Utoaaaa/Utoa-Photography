...
describe('POST /api/assets/batch-delete', () => {
  ...
  afterEach(() => {
    ...
    Object.values(db.prisma.collectionAsset).forEach((fn: any) => fn.mockReset && fn.mockReset());
  ...
  it('rejects when more than 20 ids are provided', async () => {
    ...
    const res: any = await route.POST(makeJsonRequest({ asset_ids: ids }));
  ...
  it('returns summary with partial failures (referenced)', async () => {
    ...
    const res: any = await route.POST(makeJsonRequest({ asset_ids: ['a1'] }));
  ...
  it('deletes when not referenced and logs audit', async () => {
    ...
    const res: any = await route.POST(makeJsonRequest({ asset_ids: ['a2'] }));
    expect(res.status).toBe(200);
