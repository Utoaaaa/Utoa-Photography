...
async function main() {
  ...
  if (!token) throw new Error('CF_ACCESS_TOKEN with a valid administrator Access JWT is required');
  const post = (u, d) => fetch(u, { method: 'POST', headers: { 'content-type': 'application/json', 'cf-access-token': token }, body: JSON.stringify(d) });

  const yRes = await post(`${base}/api/admin/years`, { label: 'Test Year 2024', status: 'published' });
  console.log('POST /api/admin/years status:', yRes.status);
  ...
  const c1Res = await post(`${base}/api/admin/years/${y.id}/collections`, { slug: 'probe-collection', title: 'Probe Collection', status: 'published' });
  console.log('POST /api/admin/years/{id}/collections status:', c1Res.status);
  ...
  const pageRes = await fetch(`${base}/admin/collections/${c1.id}`, { headers: { 'Cf-Access-Token': token } });
