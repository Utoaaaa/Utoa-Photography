 ...
 */
export async function d1GetYears(
  params: {
...
export async function d1CreateYear(data: {
  ...
  await db
    ...
    .bind(id, data.label, data.order_index, data.status, now, now)
    .run();
...
export async function d1CreateAsset(data: {
  ...
  await db
    .prepare(
      'INSERT INTO assets (id, alt, caption, description, title, photographer, location, tags, width, height, metadata_json, location_folder_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    )
    .bind(
      data.id,
      data.alt,
...
export async function d1GetAssetUsageCount(assetId: string): Promise<number> {
  ...
  const result = (await db
    .prepare('SELECT COUNT(*) as count FROM collection_assets WHERE asset_id = ?')
    .bind(assetId)
