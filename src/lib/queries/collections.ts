...
function mapCollectionWithCount(row: Record<string, unknown>) {
  return {
    ...
    updated_at: new Date(String(row.updated_at ?? new Date().toISOString())),
    _count: {
...
function mapAssetRow(row: Record<string, unknown>) {
  return {
    ...
    asset: {
      ...
      photographer: row.photographer != null ? String(row.photographer) : null,
      location: row.location != null ? String(row.location) : null,
      tags: row.tags != null ? String(row.tags) : null,
      width: Number(row.width),
      height: Number(row.height),
      metadata_json: row.metadata_json ?? null,
      location_folder_id: row.location_folder_id != null ? String(row.location_folder_id) : null,
