...
import { buildHomeHref, buildYearHref } from '@/lib/site-paths';
...
export default async function ClassicLocationPage({ params }: LocationPageParams) {
  const resolvedParams = await params;
  const yearLabel = decodeURIComponent(resolvedParams.year);
  const locationSlug = decodeURIComponent(resolvedParams.location);
  const result = await getLocationByYearAndSlugCached(yearLabel, locationSlug);
  ...
  const { year, location } = result;
  const homeHref = buildHomeHref(CLASSIC_BASE_PATH);
  const yearHref = buildYearHref(year.label, CLASSIC_BASE_PATH);
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-16">
        <header className="space-y-4">
          <div className="flex items-center gap-3 text-sm font-medium text-gray-500">
            <Link href={homeHref} className="text-gray-500 transition hover:text-gray-900">
              首頁
            </Link>
