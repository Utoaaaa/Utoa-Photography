...
import { buildYearAnchorId } from '@/lib/site-paths';
...
export default async function ClassicHomepage() {
  const data = await loadYearLocationData().catch((error) => {
    console.error('Failed to load year-location data for classic homepage:', error);
        ...
        <div className="text-gray-900">
          ...
          >
            <div className="px-3 py-2 md:px-4 md:py-3">
              <div className="font-serif text-2xl md:text-3xl lg:text-4xl font-semibold leading-none tracking-wide">
                Utoa
              </div>
              <div className="font-serif text-2xl md:text-3xl lg:text-4xl font-semibold leading-none tracking-wide mt-1">
                Photography
              </div>
            </div>
          </GlassSurface>
          ...
          <div className="w-full max-w-7xl mx-auto space-y-24">
            {hasYears ? (
              years.map((year) => {
                ...
                const headingId = `year-heading-${sectionId.slice('year-'.length)}`;
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                      <div>
                        <h2 id={headingId} className="font-serif text-4xl font-semibold tracking-wide text-gray-900">
                          {year.label}
                        </h2>
