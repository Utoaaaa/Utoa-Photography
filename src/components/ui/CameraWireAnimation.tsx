...
export function CameraWireAnimation({ className = '' }: CameraWireAnimationProps) {
  ...
  const lensInnerRef = useRef<SVGPathElement>(null);
  const lensKnurlsRef = useRef<SVGPathElement>(null);
  const lensHighlightRef = useRef<SVGPathElement>(null);
  const inline1Ref = useRef<SVGPathElement>(null);
  const inline2Ref = useRef<SVGPathElement>(null);
  const inline3Ref = useRef<SVGPathElement>(null);
  const inline4Ref = useRef<SVGPathElement>(null);
  const inline5Ref = useRef<SVGPathElement>(null);
    ...
    const loadAnimation = async () => {
      try {
        ...
        const toAnim = [
          ...
          inline4Ref.current,
          inline5Ref.current,
          lensOuterRef.current,
          lensMidRef.current,
          lensInnerRef.current,
          lensKnurlsRef.current,
          lensHighlightRef.current,
        ].filter(Boolean) as SVGPathElement[];

        // 用 getTotalLength() 為每條路徑設置正確的 dash 值
        const setDashByLen = (el: SVGPathElement) => {
          // 量測總長度(對多段 path 也會回傳合計長度)
        ...
        const ctx = gsap.context(() => {
          // 初始化所有路徑為描邊動畫狀態
          toAnim.forEach(setDashByLen);

          // === 載入動畫時間軸 - 放慢速度讓動畫更清晰 ===
          const loadTimeline = gsap.timeline({ defaults: { ease: 'power1.inOut' } });

          // 1. 外框描邊繪製 - 更快速
          loadTimeline.set(outlineRef.current, { opacity: 1 }, 0);
          loadTimeline.to(outlineRef.current, {
            strokeDashoffset: 0,
            duration: 2.8,
          }, 0);

          // 2. 內部線條群組 - 更快速繪製
          loadTimeline.set(inline1Ref.current, { opacity: 1 }, 0.3);
          loadTimeline.to(inline1Ref.current, {
            strokeDashoffset: 0,
            duration: 1.8,
          }, 0.3);
          
          loadTimeline.set(inline2Ref.current, { opacity: 1 }, 0.4);
          loadTimeline.to(inline2Ref.current, {
            strokeDashoffset: 0,
            duration: 2.0,
          }, 0.4);
          
          loadTimeline.set(inline3Ref.current, { opacity: 1 }, 0.5);
          loadTimeline.to(inline3Ref.current, {
            strokeDashoffset: 0,
            duration: 2.2,
          }, 0.5);
          
          loadTimeline.set(inline4Ref.current, { opacity: 1 }, 0.6);
          loadTimeline.to(inline4Ref.current, {
            strokeDashoffset: 0,
            duration: 2.4,
          }, 0.6);
          
          loadTimeline.set(inline5Ref.current, { opacity: 1 }, 0.7);
          loadTimeline.to(inline5Ref.current, {
            strokeDashoffset: 0,
            duration: 2.6,
          }, 0.7);

          // 3. 鏡頭外環更快
          loadTimeline.set(lensOuterRef.current, { opacity: 1 }, 0.6);
          loadTimeline.to(lensOuterRef.current, {
            strokeDashoffset: 0,
            duration: 2.5,
          }, 0.6);

          // 4. 鏡頭中環接著
          loadTimeline.set(lensMidRef.current, { opacity: 1 }, 0.8);
          loadTimeline.to(lensMidRef.current, {
            strokeDashoffset: 0,
            duration: 1.5,
          }, 0.8);

          // 5. 鏡頭內環跟上
          loadTimeline.set(lensInnerRef.current, { opacity: 1 }, 0.8);
          loadTimeline.to(lensInnerRef.current, {
            strokeDashoffset: 0,
            duration: 1.3,
          }, 0.8);

          // 6. 刻紋
          loadTimeline.set(lensKnurlsRef.current, { opacity: 1 }, 1.2);
          loadTimeline.to(lensKnurlsRef.current, {
            strokeDashoffset: 0,
            duration: 2.2,
          }, 1.2);

          // 7. 高光線條 - 帶透明度淡入
          loadTimeline.to(lensHighlightRef.current, {
            strokeDashoffset: 0,
            opacity: 0.7,
            duration: 3.0,
          }, 1.4);
      ...
      >
        <g id="camera-wire">
          ...
          <path
            ref={lensKnurlsRef}
            id="lensKnurls"
            d="M455 470L414 491M345 380L306.5 400M435.5 443L394.5 464M411.5 417L370.5 438M382 396L341 417M487.5 616.5L449 634.5M474 505L433 526M485 544L444 565M490 584L449 605M482 647L441.5 667M465 681.5L427.5 698M312 373.5L273.5 393.5"
            stroke="currentColor"
            strokeWidth="6"
            strokeLinecap="round"
            strokeLinejoin="round"
          ...
          <path
            ref={lensInnerRef}
            id="lensInner"
            d="M412.608 616.011C408.699 673.817 373.822 716 308.615 716C235.116 716 171 628.563 171 549.612C171 492.587 205.877 434 271.084 434C336.291 434 419.408 515.47 412.608 616.011Z"
            stroke="currentColor"
            strokeWidth="4"
            strokeLinecap="round"
            strokeLinejoin="round"
          ...
          <path
            ref={lensMidRef}
            id="lensMid"
            d="M157 546.011C157 480.898 196.056 414 269.077 414C342.098 414 435.176 507.026 427.561 621.828C423.183 687.834 384.127 736 311.106 736C228.799 736 157 636.16 157 546.011Z"
            stroke="currentColor"
            strokeWidth="6"
            strokeLinecap="round"
            strokeLinejoin="round"
          ...
          <path
            ref={lensOuterRef}
            id="lensOuter"
            d="M226.578 400.76C246.688 382.108 274.495 372 310.17 372C393.16 372 498.945 476.293 490.291 605C486.217 665.594 454.721 714.595 398.24 729.702M290.267 372.5C311.283 352.518 339.269 343 375.85 343C467.359 343 550 432.304 550 540C550 630.019 516.302 683.915 450.983 699.713M447.5 627C442.524 701 398.136 755 315.146 755C221.602 755 140 643.068 140 542C140 469 184.388 394 267.379 394C350.369 394 456.154 498.293 447.5 627Z"
            stroke="currentColor"
            strokeWidth="6"
            strokeLinecap="round"
            strokeLinejoin="round"
          ...
          <path
            ref={inline1Ref}
            d="M526 195.5L578 169.5M526 195.5L505.5 190.5M526 195.5V206.5M505.5 190.5L537.5 169.5M505.5 190.5V200.279M455.5 176L505.5 148M455.5 176L432 169.5M455.5 176L455.156 185M425.5 176L455.156 185M455.156 185L505.5 200.279"
            stroke="currentColor"
            strokeWidth="4"
            strokeLinecap="round"
            strokeLinejoin="round"
          ...
          <path
            ref={inline2Ref}
            d="M505.5 215.5L522 310.5M505.5 215.5L526 206.5M505.5 215.5L405 185M522 310.5L609.5 317.5M522 310.5L325.5 253.5M522 310.5L529 364M526 206.5L505.5 200.279"
            stroke="currentColor"
            strokeWidth="4"
            strokeLinecap="round"
            strokeLinejoin="round"
          ...
          <path
            ref={inline3Ref}
            d="M609.5 317.5L553 210.5M609.5 317.5L643 281.5M609.5 317.5L569.25 340.75M553 210.5L581.5 195.5M553 210.5L526 206.5M529 364L569.25 340.75M569.25 340.75C602 352.167 674.2 376.1 701 380.5C734.5 386 758.5 394.5 787.5 380.5C810.7 369.3 827.167 349.333 832.5 340.75"
            stroke="currentColor"
            strokeWidth="4"
            strokeLinecap="round"
            strokeLinejoin="round"
