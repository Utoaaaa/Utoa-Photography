...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 初始狀態：色塊應該是隱藏的
      gsap.set(preLayers, { visibility: 'hidden' });
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    layerStates.forEach(ls => {
      gsap.set(ls.el, { visibility: 'visible' });
    });
  ...
  return (
    <div className="sm-scope w-full h-full">
      <div
        className={(className ? className + ' ' : '') + 'staggered-menu-wrapper fixed inset-0 pointer-events-none z-40'}
      ...
      >
        ...
        >
          {(() => {
            ...
            return arr.map((c, i) => (
              <div
                key={i}
                className="sm-prelayer fixed top-0 right-0 h-full translate-x-0"
                style={{ background: c, width: 'inherit' }}
              />
            ));
          })()}
        </div>
        ...
        <aside
          ...
          ref={panelRef}
          className="staggered-menu-panel fixed top-0 right-0 h-full bg-white/95 backdrop-blur-md flex flex-col p-[8em_2.5em_3em_2.5em] overflow-y-auto z-10 pointer-events-auto"
            ...
            >
              {items && items.length ? (
                ...
                <li className="sm-panel-itemWrap relative overflow-hidden leading-none" aria-hidden="true">
                  <span className="sm-panel-item relative text-gray-900 font-serif font-light text-[3rem] md:text-[4rem] cursor-pointer leading-none tracking-tight transition-colors duration-300 ease-out inline-block no-underline">
                    <span className="sm-panel-itemLabel inline-block [transform-origin:50%_100%] will-change-transform">
                      No items
                    </span>
                  </span>
...
.sm-scope .sm-prelayer { position: fixed; top: 0; right: 0; height: 100%; width: clamp(280px, 25vw, 380px); transform: translateX(0); }
...
.sm-scope .sm-panel-list[data-numbering] .sm-panel-item::after { counter-increment: smItem; content: counter(smItem, decimal-leading-zero); position: absolute; top: 0.2em; right: -2em; font-size: 14px; font-weight: 300; color: var(--sm-accent, #666); letter-spacing: 0; pointer-events: none; user-select: none; opacity: var(--sm-num-opacity, 0); }
@media (max-width: 1024px) { .sm-scope .staggered-menu-panel { width: 100%; left: 0; right: 0; } .sm-scope .sm-prelayers { width: 100%; } }
@media (max-width: 640px) { .sm-scope .staggered-menu-panel { width: 100%; left: 0; right: 0; padding: 6em 2em 2em 2em; } .sm-scope .sm-prelayers { width: 100%; } .sm-scope .sm-panel-item { font-size: 2.5rem; } }
      `}</style>
    </div>
  );
};
